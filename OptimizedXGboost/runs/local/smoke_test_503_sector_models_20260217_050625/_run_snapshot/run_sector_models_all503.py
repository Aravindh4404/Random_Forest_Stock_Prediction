...
def metric_row(split: str, y_true: pd.Series, y_pred: np.ndarray) -> dict:
    ...
    yp = np.asarray(y_pred, dtype=float)
    acc = directional_accuracy(yt, yp)
...
def run_gridsearch_with_target(
    ...
    gs = GridSearchCV(
        ...
        scoring={
            ...
            "r2": "r2",
            "accuracy": make_scorer(directional_accuracy),
    ...
    acc_long = to_long("_test_accuracy", "accuracy")
    ...
    cv_detail = (
        ...
        .merge(acc_long, on=["candidate_id", "fold"], how="outer")
...
def run_sector_model(sector_name: str, sector_df: pd.DataFrame, out_dir: Path) -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    val_pred["direction_correct"] = (val_pred["pred_direction"] == val_pred["actual_direction"]).astype(int)
...
def main():
    ...
    if not summary_df.empty:
        ...
        lines.append(
            summary_df[["sector", "n_tickers", "val_rmse", "val_mae", "val_r2", "val_accuracy"]]
            .head(10)
